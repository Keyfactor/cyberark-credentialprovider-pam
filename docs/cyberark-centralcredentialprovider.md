...
If certificate authentication is required for your CyberArk Credential Provider service, please use the `CyberArk-ClientAuth-CentralCredentialProvider` PAM type. If you are using this PAM provider type, Certificate Authentication needs to be disabled. This may necessitate creating a Site that allows HTTPS requests but does not require a Client Certificate to authenticate. By default the site `AIMWebService` may require a Client Certificate, which would need to be edited or have another site created.
